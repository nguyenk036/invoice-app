...
// ── Blank form ─────────────────────────────────────────────────
function blankForm(taxRate = 13): InvoiceForm {
  ...
  return (
    <div className="grid grid-cols-[1fr_80px_100px_80px_32px] gap-2 items-start">
      ...
      <input
        ...
        className="px-2.5 py-1.5 text-sm text-slate-700 bg-white border border-slate-200 rounded-lg outline-none focus:border-sky-400 transition-all text-right"
      />
      <input
        type="number"
        min="0"
...
function InvoiceDrawer({
  ...
  const taxAmt = (subtotal - discount) * (Number(form.tax_rate) / 100);
...
export default function InvoicesPage() {
  ...
  const updateInvoice = useUpdateInvoice(editingId ?? '');

  const filtered = useMemo(() => {
    if (statusFilter === 'all') return invoices as InvoiceRow[];
    return (invoices as InvoiceRow[]).filter((i) => i.status === statusFilter);
  ...
  function openEdit(inv: InvoiceRow) {
    setForm({
      ...
      line_items: [blankLine()], // line items need separate fetch — simplification
    });
    setEditingId(inv.id);
    setFormError(null);
    setDrawerMode('edit');
  ...
  const statusFilters: Array<{ key: InvoiceStatus | 'all'; label: string }> = [
    { key: 'all', label: 'All' },
    ...
    >
        ...
        <main className="flex-1 px-4 sm:px-6 lg:px-8 py-6 max-w-7xl w-full mx-auto space-y-4">
            ...
            {[
              {
                ...
                value: fmt(totals.all, currency),
                color: 'text-slate-900',
              },
              {
                label: 'Collected',
                value: fmt(totals.paid, currency),
                color: 'text-emerald-600',
              },
              {
                label: 'Outstanding',
                value: fmt(totals.pending, currency),
              ...
              >
                <div className="text-slate-400 text-xs mb-1.5">{c.label}</div>
          ...
          {!isLoading && filtered.length > 0 && (
            <div className="bg-white rounded-2xl border border-slate-100 overflow-hidden">
              {/* Desktop header */}
              <div className="hidden sm:grid grid-cols-[2fr_1fr_1fr_120px_100px_80px] gap-4 px-5 py-3 border-b border-slate-50 bg-slate-50/60">
                {['Client', 'Invoice #', 'Issued', 'Status', 'Amount', ''].map(
                  (h) => (
                    <span
                      key={h}
                      className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider last:text-right"
                    >
                      {h}
                    </span>
                  )
              ...
              <div className="divide-y divide-slate-50">
                {filtered.map((inv) => (
                  <div
                    ...
                    className="flex sm:grid sm:grid-cols-[2fr_1fr_1fr_120px_100px_80px] gap-3 sm:gap-4 items-center px-5 py-3.5 hover:bg-slate-50/60 transition-colors cursor-pointer group"
                  >
                    {/* Client */}
                    <div className="flex items-center gap-3 min-w-0">
                      <div className="w-8 h-8 rounded-xl bg-sky-50 text-sky-500 ring-1 ring-sky-200 flex items-center justify-center shrink-0">
                        <IcFile s={13} />
                      </div>
                      <div className="min-w-0">
                        ...
                        {inv.due_date && (
                          <div
                            className={`text-xs ${inv.status === 'overdue' ? 'text-red-500' : 'text-slate-400'}`}
                          >
      ...
      {deleteTarget && (
        <DeleteModal
          ...
          message="This invoice will be permanently removed along with its line items."
          onConfirm={() =>
