  ...
  return useMutation({
    mutationFn: async ({
      ...
      if (line_items.length > 0) {
        ...
        const { error: liErr } =
          await from<LineItem>('line_items').insert(rows);
  ...
  return useQuery<DashboardStats>({
    ...
    queryFn: async () => {
      ...
      const [allInvoicesRes, clientCountRes, recentRes] = await Promise.all([
        from<Pick<Invoice, 'status' | 'total' | 'paid_at'>>('invoices').select(
      ...
      const thisMonthRevenue = invoices
        .filter(
          (i) =>
            i.status === 'paid' && i.paid_at != null && i.paid_at >= monthStart
        )
      ...
      const paidThisYear = invoices
        .filter(
          (i) =>
            i.status === 'paid' && i.paid_at != null && i.paid_at >= yearStart
        )
        .reduce((s, i) => s + (i.total ?? 0), 0);
